var planView = Backbone.View.extend({
    ...
    template: '/blueloop-backend/static/js/admin/billing/template/planTemplate.ejs',
	...
	addSilverPlan: function(e){
		if(window.wsView.model.get("token") == undefined){
			...
			var data = ajaxCall('POST','/blueloop-backend/administrator/sellPlan/', JSON.stringify({token:window.wsView.model.get("token")}), "text/json", "json", false);
    		if(data.success){
