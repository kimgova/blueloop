...
$.fx.multipleValueStep._default)(fx);funcs[fx.prop]=[];$.each(fx.values,function(i,val){funcs[fx.prop].push(val.now+($.cssNumber[fx.prop]?"":val.unit))})}else funcs[fx.prop]=fx.now+($.cssNumber[fx.prop]?"":fx.unit);transform.exec(funcs,{preserve:true})}});$.each(["matrix","reflect","reflectX","reflectXY","reflectY"],function(i,func){$.fx.step[func]=function(fx){var transform=fx.elem.transform||new $.transform(fx.elem),funcs={};if(!fx.initialized){fx.initialized=true;if(func!=="matrix"){var values=
$.matrix[func]().elements;var val;$.each(fx.values,function(i){switch(i){case 0:val=values[0];break;case 1:val=values[2];break;case 2:val=values[1];break;case 3:val=values[3];break;default:val=0}fx.values[i].end=val})}fx.decomposed={};var v=fx.values;fx.decomposed.start=$.matrix.matrix(v[0].start,v[1].start,v[2].start,v[3].start,v[4].start,v[5].start).decompose();fx.decomposed.end=$.matrix.matrix(v[0].end,v[1].end,v[2].end,v[3].end,v[4].end,v[5].end).decompose()}($.fx.multipleValueStep[fx.prop]||
$.fx.multipleValueStep._default)(fx);funcs.matrix=[];$.each(fx.values,function(i,val){funcs.matrix.push(val.now)});transform.exec(funcs,{preserve:true})}})})(jQuery,this,this.document);
(function($,window,document,undefined){var RAD_DEG=180/Math.PI;var RAD_GRAD=200/Math.PI;var DEG_RAD=Math.PI/180;var DEG_GRAD=2/1.8;var GRAD_DEG=.9;var GRAD_RAD=Math.PI/200;var rfxnum=/^([+\-]=)?([\d+.\-]+)(.*)$/;$.extend({angle:{runit:/(deg|g?rad)/,radianToDegree:function(rad){return rad*RAD_DEG},radianToGrad:function(rad){return rad*RAD_GRAD},degreeToRadian:function(deg){return deg*DEG_RAD},degreeToGrad:function(deg){return deg*DEG_GRAD},gradToDegree:function(grad){return grad*GRAD_DEG},gradToRadian:function(grad){return grad*
GRAD_RAD},toDegree:function(val){var parts=rfxnum.exec(val);if(parts){val=parseFloat(parts[2]);switch(parts[3]||"deg"){case "grad":val=$.angle.gradToDegree(val);break;case "rad":val=$.angle.radianToDegree(val);break}return val}return 0}}})})(jQuery,this,this.document);
(function($,window,document,undefined){if(typeof $.matrix=="undefined")$.extend({matrix:{}});var $m=$.matrix;$.extend($m,{V2:function(x,y){if($.isArray(arguments[0]))this.elements=arguments[0].slice(0,2);else this.elements=[x,y];this.length=2},V3:function(x,y,z){if($.isArray(arguments[0]))this.elements=arguments[0].slice(0,3);else this.elements=[x,y,z];this.length=3},M2x2:function(m11,m12,m21,m22){if($.isArray(arguments[0]))this.elements=arguments[0].slice(0,4);else this.elements=Array.prototype.slice.call(arguments).slice(0,
4);this.rows=2;this.cols=2},M3x3:function(m11,m12,m13,m21,m22,m23,m31,m32,m33){if($.isArray(arguments[0]))this.elements=arguments[0].slice(0,9);else this.elements=Array.prototype.slice.call(arguments).slice(0,9);this.rows=3;this.cols=3}});var Matrix={e:function(row,col){var rows=this.rows,cols=this.cols;if(row>rows||col>rows||row<1||col<1)return 0;return this.elements[(row-1)*cols+col-1]},decompose:function(){var a=this.e(1,1),b=this.e(2,1),c=this.e(1,2),d=this.e(2,2),e=this.e(1,3),f=this.e(2,3);
if(Math.abs(a*d-b*c)<.01)return{rotate:0+"deg",skewX:0+"deg",scaleX:1,scaleY:1,translateX:0+"px",translateY:0+"px"};var tx=e,ty=f;var sx=Math.sqrt(a*a+b*b);a=a/sx;b=b/sx;var k=a*c+b*d;c-=a*k;d-=b*k;var sy=Math.sqrt(c*c+d*d);c=c/sy;d=d/sy;k=k/sy;if(a*d-b*c<0){a=-a;b=-b;sx=-sx}var rad2deg=$.angle.radianToDegree;var r=rad2deg(Math.atan2(b,a));k=rad2deg(Math.atan(k));return{rotate:r+"deg",skewX:k+"deg",scaleX:sx,scaleY:sy,translateX:tx+"px",translateY:ty+"px"}}};$.extend($m.M2x2.prototype,Matrix,{toM3x3:function(){var a=
this.elements;return new $m.M3x3(a[0],a[1],0,a[2],a[3],0,0,0,1)},x:function(matrix){var isVector=typeof matrix.rows==="undefined";if(!isVector&&matrix.rows==3)return this.toM3x3().x(matrix);var a=this.elements,b=matrix.elements;if(isVector&&b.length==2)return new $m.V2(a[0]*b[0]+a[1]*b[1],a[2]*b[0]+a[3]*b[1]);else if(b.length==a.length)return new $m.M2x2(a[0]*b[0]+a[1]*b[2],a[0]*b[1]+a[1]*b[3],a[2]*b[0]+a[3]*b[2],a[2]*b[1]+a[3]*b[3]);return false},inverse:function(){var d=1/this.determinant(),a=this.elements;
return new $m.M2x2(d*a[3],d*-a[1],d*-a[2],d*a[0])},determinant:function(){var a=this.elements;return a[0]*a[3]-a[1]*a[2]}});$.extend($m.M3x3.prototype,Matrix,{x:function(matrix){var isVector=typeof matrix.rows==="undefined";if(!isVector&&matrix.rows<3)matrix=matrix.toM3x3();var a=this.elements,b=matrix.elements;if(isVector&&b.length==3)return new $m.V3(a[0]*b[0]+a[1]*b[1]+a[2]*b[2],a[3]*b[0]+a[4]*b[1]+a[5]*b[2],a[6]*b[0]+a[7]*b[1]+a[8]*b[2]);else if(b.length==a.length)return new $m.M3x3(a[0]*b[0]+
...
d*(a[7]*a[3]-a[6]*a[4]),d*-(a[7]*a[0]-a[6]*a[1]),d*(a[4]*a[0]-a[3]*a[1]))},determinant:function(){var a=this.elements;return a[0]*(a[8]*a[4]-a[7]*a[5])-a[3]*(a[8]*a[1]-a[7]*a[2])+a[6]*(a[5]*a[1]-a[4]*a[2])}});var Vector={e:function(i){return this.elements[i-1]}};$.extend($m.V2.prototype,Vector);$.extend($m.V3.prototype,Vector)})(jQuery,this,this.document);
(function($,window,document,undefined){if(typeof $.matrix=="undefined")$.extend({matrix:{}});$.extend($.matrix,{calc:function(matrix,outerHeight,outerWidth){this.matrix=matrix;this.outerHeight=outerHeight;this.outerWidth=outerWidth}});$.matrix.calc.prototype={coord:function(x,y,z){z=typeof z!=="undefined"?z:0;var matrix=this.matrix,vector;switch(matrix.rows){case 2:vector=matrix.x(new $.matrix.V2(x,y));break;case 3:vector=matrix.x(new $.matrix.V3(x,y,z));break}return vector},corners:function(x,y){var save=
!(typeof x!=="undefined"||typeof y!=="undefined"),c;if(!this.c||!save){y=y||this.outerHeight;x=x||this.outerWidth;c={tl:this.coord(0,0),bl:this.coord(0,y),tr:this.coord(x,0),br:this.coord(x,y)}}else c=this.c;if(save)this.c=c;return c},sides:function(corners){var c=corners||this.corners();return{top:Math.min(c.tl.e(2),c.tr.e(2),c.br.e(2),c.bl.e(2)),bottom:Math.max(c.tl.e(2),c.tr.e(2),c.br.e(2),c.bl.e(2)),left:Math.min(c.tl.e(1),c.tr.e(1),c.br.e(1),c.bl.e(1)),right:Math.max(c.tl.e(1),c.tr.e(1),c.br.e(1),
c.bl.e(1))}},offset:function(corners){var s=this.sides(corners);return{height:Math.abs(s.bottom-s.top),width:Math.abs(s.right-s.left)}},area:function(corners){var c=corners||this.corners();var v1={x:c.tr.e(1)-c.tl.e(1)+c.br.e(1)-c.bl.e(1),y:c.tr.e(2)-c.tl.e(2)+c.br.e(2)-c.bl.e(2)},v2={x:c.bl.e(1)-c.tl.e(1)+c.br.e(1)-c.tr.e(1),y:c.bl.e(2)-c.tl.e(2)+c.br.e(2)-c.tr.e(2)};return.25*Math.abs(v1.e(1)*v2.e(2)-v1.e(2)*v2.e(1))},nonAffinity:function(){var sides=this.sides(),xDiff=sides.top-sides.bottom,yDiff=
sides.left-sides.right;return parseFloat(parseFloat(Math.abs((Math.pow(xDiff,2)+Math.pow(yDiff,2))/(sides.top*sides.bottom+sides.left*sides.right))).toFixed(8))},originOffset:function(toOrigin,fromOrigin){toOrigin=toOrigin?toOrigin:new $.matrix.V2(this.outerWidth*.5,this.outerHeight*.5);fromOrigin=fromOrigin?fromOrigin:new $.matrix.V2(0,0);var toCenter=this.coord(toOrigin.e(1),toOrigin.e(2));var fromCenter=this.coord(fromOrigin.e(1),fromOrigin.e(2));return{top:fromCenter.e(2)-fromOrigin.e(2)-(toCenter.e(2)-
toOrigin.e(2)),left:fromCenter.e(1)-fromOrigin.e(1)-(toCenter.e(1)-toOrigin.e(1))}}}})(jQuery,this,this.document);
(function($,window,document,undefined){if(typeof $.matrix=="undefined")$.extend({matrix:{}});var $m=$.matrix,$m2x2=$m.M2x2,$m3x3=$m.M3x3;$.extend($m,{identity:function(size){size=size||2;var length=size*size,elements=new Array(length),mod=size+1;for(var i=0;i<length;i++)elements[i]=i%mod===0?1:0;return new $m["M"+size+"x"+size](elements)},matrix:function(){var args=Array.prototype.slice.call(arguments);switch(arguments.length){case 4:return new $m2x2(args[0],args[2],args[1],args[3]);case 6:return new $m3x3(args[0],
args[2],args[4],args[1],args[3],args[5],0,0,1)}},reflect:function(){return new $m2x2(-1,0,0,-1)},reflectX:function(){return new $m2x2(1,0,0,-1)},reflectXY:function(){return new $m2x2(0,1,1,0)},reflectY:function(){return new $m2x2(-1,0,0,1)},rotate:function(deg){var rad=$.angle.degreeToRadian(deg),costheta=Math.cos(rad),sintheta=Math.sin(rad);var a=costheta,b=sintheta,c=-sintheta,d=costheta;return new $m2x2(a,c,b,d)},scale:function(sx,sy){sx=sx||sx===0?sx:1;sy=sy||sy===0?sy:sx;return new $m2x2(sx,
0,0,sy)},scaleX:function(sx){return $m.scale(sx,1)},scaleY:function(sy){return $m.scale(1,sy)},skew:function(degX,degY){degX=degX||0;degY=degY||0;var radX=$.angle.degreeToRadian(degX),radY=$.angle.degreeToRadian(degY),x=Math.tan(radX),y=Math.tan(radY);return new $m2x2(1,x,y,1)},skewX:function(degX){return $m.skew(degX)},skewY:function(degY){return $m.skew(0,degY)},translate:function(tx,ty){tx=tx||0;ty=ty||0;return new $m3x3(1,0,tx,0,1,ty,0,0,1)},translateX:function(tx){return $m.translate(tx)},translateY:function(ty){return $m.translate(0,
ty)}})})(jQuery,this,this.document);